...
if length(setup.number_blocks)>1
...
Hess{bb}=.5*eye(length(setup.index_block{bb}));
    ...
    try
        ...
        Hess_temp = hessian3(func2str(@postblock),xestimate(1:(min(setup.weight_index)-1)),setup,data,1,xestimate);

   Hess_temp=reshape(-Hess_temp,length(1:(min(setup.weight_index)-1)),length(1:min(setup.weight_index)-1));
  Hess_temp=eye(size(Hess_temp,1))/Hess_temp; 
  ...
  [V,D]=eig(Hess_temp);
   ...
   Hess_temp= V*diag(d)*V';
   Hess{1}=.75*Hess_temp+.25*max(diag(Hess_temp))*eye(length(1:min(setup.weight_index)-1));
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     catch ME
         disp('Hessian computation in the following block did not work:')
         
Hess{1}=.5*eye(length(1:(min(setup.weight_index)-1)));
     end 
