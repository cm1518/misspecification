...
alpha=min(1,exp(post_prop-old_posterior+adjustment( posterior_draw,posterior_draw_prop,setup ))*dirpdf(posterior_draw(setup.weight_index),setup.dirichlet_proposal_parameters)/dirpdf(posterior_draw_prop(setup.weight_index),setup.dirichlet_proposal_parameters));
elseif block<setup.number_blocks %a non-dirichlet block
alpha=min(1,exp(post_prop-old_posterior+adjustment( posterior_draw,posterior_draw_prop,setup )));
elseif block==setup.number_blocks %the weight block
    prop_adjust=dirpdf(posterior_draw(setup.weight_index),setup.dirichlet_proposal_parameters)/dirpdf(posterior_draw_prop(setup.weight_index),setup.dirichlet_proposal_parameters);
