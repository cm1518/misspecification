...
setup.number_of_draws=20000;%500000
...
setup.index_normal=[1;2;3;4]; % index for parameters that have normal priors
setup.normal_prior_means=[0;0;0;.5]; %prior means
setup.normal_prior_std=[1;1;1;.1]; % prior std
...
setup.index_block{1}=[1:4]';

%information on the models
setup.number_models=2; %number of models
setup.index_model{1}=[1 2]; %parameter indices for different models - there are two more set of indices than there are 
%models - the first set of indices refers to the joint parameters, the last set to the model weights (there is one less
	%model weight than there are model to impose that the model weights sum to one)
setup.index_model{2}=[];
setup.index_model{3}=[3];
setup.index_model{4}=[4];
 %model weights are now estimated

setup.initial_parameter=[.8;.4;.4;.5];
